...
export default function SaveWeeklyPalette() {
    ...
    return (
        <div className="w-11/12 md:w-4/5 lg:w-3/4 xl:w-3/5 max-w-3xl mx-auto">
            ...
            <p className="text-zinc-300 mb-6">My wall of colors, updated weekly. Read more about it <Link href="/posts/52-weeks-colors" className="link">here</Link>.</p>
            {!apiKey ? (
                <div className="mb-6">
                    <p className="text-zinc-300 text-sm mb-3">API key required to save palettes.<button onClick={() => setShowApiKeyInput(!showApiKeyInput)} className="link">Click here</button> to set it up.</p>{showApiKeyInput && <ApiKeyInput onSave={saveApiKey} />}
                </div>
            ) : (
            ...
            {showApiKeyInput && apiKey && (
                <div className="mb-6">
                    <ApiKeyInput onSave={saveApiKey} initialValue={apiKey} />
                </div>
            )}
                            ...
                            {exists && palette ? (
                                ...
                                <div className="p-0 overflow-hidden rounded-none">
                                    <div className="flex items-center justify-center">
                                        <button onClick={() => saveForWeek(week)} disabled={isLoading || !apiKey} className={`bg-zinc-800 w-full aspect-10/1 text-sm font-medium transition-colors ${isLoading ? 'cursor-wait' : !apiKey ? 'cursor-not-allowed' : ''}`}>
                                            {isLoading ? 'Saving...' : `Save Palette for Week ${week}`}
                                        </button>
                                    </div>
                                </div>
                            )}
    ...
    return (
        <div>
            ...
            <div className="flex gap-2">
                <input type="password" value={key} onChange={(e) => setKey(e.target.value)} placeholder="Paste your API key..." className="flex-1 bg-zinc-900 text-white px-3 py-2 rounded text-sm" onKeyDown={(e) => e.key === 'Enter' && onSave(key)} />
