...
const TopArtists: React.FC = () => {
    ...
    useEffect(() => {
        axios
            ...
            .then((res) => setTopArtists(res.data.weeklyartistchart.artist));
    }, [LASTFM_KEY]);

    if (!topArtists.length) return <div>No top artists</div>;
            ...
            <div className="grid grid-flow-col gap-2 overflow-x-auto my-2">
                {topArtists.map((artist: any, index: number) => (
                    <a key={index} href={artist.url} target="_blank" className="relative w-[100px] h-[100px] overflow-hidden group">
                        {/* <Image className="rounded-md" src={artist.image[0]['#text']} alt="" width={100} height={100} /> */}
