...
const TopTracks: React.FC = () => {
    ...
    useEffect(() => {
        axios
            ...
            .then((res) => setTopTracks(res.data.weeklytrackchart.track));
    }, [LASTFM_KEY]);

    if (!topTracks.length) return <div>No top tracks</div>;
            ...
            <div className="grid grid-flow-col gap-2 overflow-x-auto my-2">
                {topTracks.map((track: any, index: number) => (
                    <a key={index} href={track.url} target="_blank" className="relative w-[100px] h-[100px] overflow-hidden group">
                        <Image className="rounded-md" src={track.image[2]['#text']} alt="" width={100} height={100} />
