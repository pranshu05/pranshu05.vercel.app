        ...
        <div className="columns-2 md:columns-3 gap-2 md:gap-4">
            {imageData.map((image, index) => (
                <div key={index} className='mb-2 md:mb-4' onClick={() => openFullscreen(index)}>
                    <Image className={`w-full h-auto rounded-lg grayscale transition-all duration-500 ease-in-out transform hover:grayscale-0 cursor-pointer ${isLoading ? "blur-2xl" : "blur-0"}`} onLoadingComplete={() => setLoading(false)} width={1000} height={1000} src={image.urls.regular} alt="" loading="lazy" />
                </div>
            ))}
            {selectedImageIndex !== null && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop backdrop-blur-sm" onClick={closeFullscreen}>
                    <div className="relative h-dvh w-dvw flex items-center">
                        <Image className={`max-w-full max-h-full w-auto h-auto my-auto mx-auto rounded-lg ${isLoading ? "blur-2xl" : "blur-0"}`} onLoadingComplete={() => setLoading(false)} width={1000} height={1000} src={imageData[selectedImageIndex]?.urls.regular} alt="" />
