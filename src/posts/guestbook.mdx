...
readTime: 6
img: https://github.com/pranshu05/pranshu05/assets/70943732/eb40599e-29f8-4afe-9e59-c7b14cec644c
---
...
const GitHubLogin: React.FC<GitHubLoginProps> = ({ setErrorMsg }) => {
    ...
    return (
        <button onClick={handleGitHubLogin} className='py-2 px-4 rounded-md bg-black text-white flex gap-2 items-center border border-[#b4befe]'>
            <FaGithub />
            Sign in
        </button>
    );
...
const GoogleLogin: React.FC<GoogleLoginProps> = ({ setErrorMsg }) => {
    ...
    return (
        <button onClick={handleGoogleLogin} className='py-2 px-4 rounded-md bg-black text-white flex gap-2 items-center border border-[#b4befe]'>
            <FaGoogle />
            Sign in
        </button>
    );
        ...
        <div>
            {isSignedIn ? (
                <div className='flex items-baseline gap-2'>
                    ...
                    <button onClick={handleLogOut} className='py-2 px-4 rounded-md bg-black text-white flex gap-2 items-center border border-[#b4befe]'>
                        Sign out
                    </button>
                </div>
            ) : (
...
const PostMessage: React.FC<PostMessageProps> = ({ user }) => {
    ...
    return (
        <div className="mt-4 p-4 bg-black bg-opacity-40 backdrop backdrop-blur-sm rounded-md w-full border border-[#b4befe]">
            <h2 className="text-xl font-bold mb-2">Post a Message</h2>
            <textarea
                value={newMessage}
                onChange={(e) => setNewMessage(e.target.value)}
                className="w-full min-h-20 p-2 border border-[#b4befe] bg-transparent rounded-md"
                placeholder="Write your message here..."
            />
            <button
                onClick={handlePostMessage}
                className="mt-2 bg-black text-white p-2 rounded-md border border-[#b4befe]"
            >
                Post Message
            </button>
            {showWarning && (
    ...
    return (
        <div>
            ...
            {messages.map((message) => (
                <div key={message.id} className="border border-[#b4befe] rounded-md bg-transparent p-4 my-2">
