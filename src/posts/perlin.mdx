...
2. **Dot Product:**
    For any given point in space, the value of the Perlin Noise function is computed based on the dot product of the gradient vector and the distance vector from the grid point to the input point. If we denote the gradient vector at point <InlineMath math="(i, j)" /> as <InlineMath math="\mathbf{g}_{i,j}" /> and the distance vector as <InlineMath math="\mathbf{r}_{i,j}" />, the noise value at the input point <InlineMath math="(x, y)" /> is given by:

    <BlockMath math="N(x, y) = \mathbf{g}_{i,j} \cdot \mathbf{r}_{i,j}" />

     For a point in a two dimension space, the gradient vectors at the four nearest grid points are used to compute the noise value. In general terms the complexity of `dot product` is <InlineMath math="O(2^d)" /> where <InlineMath math="d" /> is the dimension of the space.
...
3. **Interpolation:**
    ...
    If we denote the noise value at the four nearest grid points as <InlineMath math="N_{i,j}" />, the noise value at the input point <InlineMath math="(x, y)" /> is given by:
    
    <BlockMath math="N(x, y) = \text{interpolate}(N_{i,j}, N_{i+1,j}, N_{i,j+1}, N_{i+1,j+1})" />
